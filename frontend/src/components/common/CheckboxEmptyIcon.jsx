/*
 * The product's empty (unchecked) checkbox glyph, from the design system.
 * It's a square 18x18 viewBox, so sizing it with the usual `w-4 h-4` keeps
 * it centred inside its box with no optical offset; the path is filled with
 * `currentColor`, so colour comes from a `text-*` class.
 */
const CheckboxEmptyIcon = ({ className = "w-4 h-4", size, style, ...props }) => (
  <svg
    viewBox="0 0 18 18"
    fill="none"
    xmlns="http://www.w3.org/2000/svg"
    className={className}
    style={size != null ? { width: size, height: size, ...style } : style}
    aria-hidden="true"
    focusable="false"
    {...props}
  >
    <path
      d="M2 18C1.45 18 0.979167 17.8042 0.5875 17.4125C0.195833 17.0208 0 16.55 0 16V2C0 1.45 0.195833 0.979167 0.5875 0.5875C0.979167 0.195833 1.45 0 2 0H16C16.55 0 17.0208 0.195833 17.4125 0.5875C17.8042 0.979167 18 1.45 18 2V16C18 16.55 17.8042 17.0208 17.4125 17.4125C17.0208 17.8042 16.55 18 16 18H2ZM2 16H16V2H2V16Z"
      fill="currentColor"
    />
  </svg>
);

export default CheckboxEmptyIcon;
