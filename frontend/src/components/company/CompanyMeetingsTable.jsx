...
const CompanyMeetingsTable = ({ companyId, setMeetings }) => {
  const [meetings, setLocalMeetings] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setLocalMeetings(response.data.meetings);
        setMeetings(response.data.meetings); // Update parent state

        // Fetch users for participants dropdown
        const usersResponse = await API.get(`/contacts/company/${companyId}`);
        ...
        setError("Failed to load meetings or users");
        console.error("Error fetching data:", err);
        toast.error("Failed to load meetings or users.");
      } finally {
        setLoading(false);
      }
    };
  ...
  const getPriorityIndicator = (priority) => {
    if (priority === 'high') {
      return <div className="w-1 h-full bg-gray-900 rounded-l-lg absolute left-0 top-0"></div>;
    }
    return null;
  };
      ...
      {filteredMeetings.length === 0 ? (
        <div className="text-center py-12 bg-gray-50 rounded-lg border border-gray-200">
          ...
          <p className="text-xs text-gray-500">
            {statusFilter ? "Try changing the filter" : "Meetings will appear here once scheduled"}
          </p>
        </div>
      ) : (
        <div className="space-y-3">
            ...
            return (
              ...
              >
                ...
                <div className="flex items-center justify-between mt-2 text-xs">
                  ...
                  {meeting.priority && (
                    <span className={`font-medium capitalize ${meeting.priority === 'high' ? 'text-gray-900' :
                      meeting.priority === 'low' ? 'text-gray-500' :
                        'text-gray-600'
                      }`}>
      ...
      {filteredMeetings.length > 0 && (
        <div className="grid grid-cols-2 gap-3 mt-4">
          <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
            <div className="text-gray-600 text-xs font-medium mb-1">Scheduled</div>
          ...
          <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
            <div className="text-gray-600 text-xs font-medium mb-1">Cancelled</div>
            <div className="text-gray-900 text-xl font-bold">
              {meetings.filter(m => m.status === 'cancelled').length}
            </div>
          </div>
          <div className="bg-gray-50 p-3 rounded-lg border border-gray-200">
            <div className="text-gray-600 text-xs font-medium mb-1">No Show</div>
            <div className="text-gray-900 text-xl font-bold">
