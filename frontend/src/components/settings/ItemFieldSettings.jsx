...
const ItemFieldSettings = () => {
  ...
  const fetchFields = async () => {
    try {
      setLoading(true);
      const res = await API.get("/item-fields");
      if (res.data) {
      ...
      console.error("Failed to fetch item fields", err);
      toast.error("Failed to load item fields");
    } finally {
      setLoading(false);
    }
  };
  ...
  const saveFields = async (updatedFields, categoriesToSave = availableCategories) => {
    try {
      ...
      if (fieldDocId) {
        await API.put(`/item-fields/${fieldDocId}`, payload);
      } else {
        const res = await API.post("/item-fields", payload);
        setFieldDocId(res.data._id);
      }
  ...
  const handleUpdateCategory = async (oldCategoryName) => {
    ...
    try {
      const res = await API.put("/item-fields/categories", {
        oldCategoryName,
  ...
  const confirmDeleteCategory = async () => {
    ...
    try {
      const res = await API.delete(`/item-fields/categories/${encodeURIComponent(categoryName)}`);
      // Update state with fresh data from backend
      setAvailableCategories(res.data.categories);
      setFields(res.data.fields);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading item fields...</p>
        </div>
      </div>
    );
  }
  ...
  const renderFieldItem = (field, index, isCategorized) => (
    <div
      ...
      onDragStart={(e) => {
        e.dataTransfer.setData("fieldIndex", index); // Native browser memory
        setDraggedFieldIndex(index); // Visual styling
      }}
  ...
  return (
    <div className="space-y-6">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                  ...
                  <span className="font-bold text-gray-900">GST Rate</span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full border border-blue-200">
                    Number
                  </span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full border border-gray-200">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                  ...
                  <span className="font-bold text-gray-900">Barcode</span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full border border-blue-200">
                    String (Single-line)
                  </span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full border border-gray-200">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-2">
              <div className="flex-1">
                <div className="flex flex-wrap items-center gap-2 sm:gap-3 mb-2">
                  ...
                  <span className="font-bold text-gray-900">Primary Unit</span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-blue-100 text-blue-700 text-xs font-semibold rounded-full border border-blue-200">
                    Dropdown
                  </span>
                  <span className="inline-flex items-center gap-1 px-2.5 py-0.5 bg-gray-100 text-gray-700 text-xs font-semibold rounded-full border border-gray-200">
        ...
        <div className="mt-4 bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
          <div className="flex flex-col sm:flex-row items-start gap-3">
            ...
            <p className="text-sm text-blue-700">
              These are built-in system fields that appear by default in every item form. You cannot edit or remove these fields, but you can add custom fields below.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g., Brand, Warranty Period, Supplier Code"
                value={newField.name}
                onChange={(e) =>
              ...
              <div className="flex gap-2 mb-3">
                <input
                  ...
                  placeholder={`Add option(s) - comma-separated (e.g., ${newField.type === "dropdown"
                    ? "Net 30, Net 60, Net 90, Immediate"
      ...
      {/* Configured Fields Grouped by Categories (Drop Zones) */}
          ...
          <div className="space-y-6">
            ...
            {fields.filter(f => !f.category || f.category === 'Uncategorized').length > 0 && (
              <div
                ...
                onDrop={(e) => handleDrop(e, "Uncategorized")} // <-- FIX: pass the 'e' event
        ...
        <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <ul className="text-sm text-blue-700 space-y-1 leading-relaxed grid grid-cols-1 md:grid-cols-2 gap-2">
              <li>
                • <strong>Text:</strong> Multi-line text area
              </li>
              <li>
                • <strong>String:</strong> Single-line input
              </li>
              <li>
                • <strong>Number:</strong> Numeric values only
              </li>
              <li>
                • <strong>Dropdown:</strong> Select one option
              </li>
              <li>
                • <strong>URL:</strong> Web address field
              </li>
              <li>
                • <strong>Date:</strong> Date picker
              </li>
              <li>
                • <strong>Multi-Select:</strong> Multiple checkboxes
              </li>
            </ul>
          </div>
        </div>
...
export default ItemFieldSettings;
