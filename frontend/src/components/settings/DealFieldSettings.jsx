...
const DealFieldSettings = () => {
  ...
  const fetchFields = async () => {
    try {
      ...
      if (res.data) {
        ...
        setAvailableCategories(res.data.fieldCategories || []); // 👉 Load categorie
        setFieldDocId(res.data._id);
      }
    } catch (err) {
      console.error("Failed to fetch deal fields", err);
      toast.error("Failed to load deal fields");
    } finally {
      setLoading(false);
    }
  };

  // 👉 Updated to accept categoriesToSave
  const saveFields = async (updatedFields, categoriesToSave = availableCategories) => {
    try {
      ...
      if (fieldDocId) {
        await API.put(`/deal-fields`, payload);
      } else {
        const res = await API.post("/deal-fields", payload);
        setFieldDocId(res.data._id);
      }
  ...
  const handleUpdateCategory = async (oldCategoryName) => {
    ...
    try {
      const res = await API.put("/deal-fields/categories", {
        oldCategoryName,
  ...
  const handleDeleteCategory = async (categoryName) => {
    ...
    try {
      const res = await API.delete(`/deal-fields/categories/${encodeURIComponent(categoryName)}`);
      // Update state with fresh data from backend
      setAvailableCategories(res.data.categories);
      setFields(res.data.fields);
  ...
  const handleAdd = async () => {
    ...
    // Split field names by comma and trim whitespace (PRESERVED)
    ...
    // Check if dropdown/multiselect has options (PRESERVED)
    if (
    ...
    // Create multiple fields from comma-separated names (PRESERVED)
    const newFieldsToAdd = fieldNames.map((fieldName) => ({
      name: fieldName,
      type: newField.type,
      required: newField.required,
      category: assignedCategory, // 👉 ADDED: Assign the category to the new field
      ...(newField.type === "dropdown" || newField.type === "multiselect"
        ? { options: newField.options }
        : {}),
    }));

    // 👉 ADDED: Ensure the category exists in the available categories list
    ...
    // 👉 ADDED: Pass the updated categories to saveFields
    const fileSaved = await saveFields(updated, updatedCategories);
    ...
    return (
      <div className="flex items-center justify-center h-64">
        <div className="text-center">
          ...
          <p className="text-gray-600">Loading deal fields...</p>
        </div>
      </div>
    );
  }

  // 👉 Abstracted Field Item Renderer for easier DND mapping
  ...
  return (
    <div className="space-y-6">
        ...
        <div className="mt-4 bg-blue-50 border-2 border-blue-200 rounded-xl p-4">
          <div className="flex flex-col sm:flex-row items-start gap-3">
            ...
            <p className="text-sm text-blue-700">
              These are built-in system fields that appear by default in every deal form. You cannot edit or remove these fields, but you can add custom fields below.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                placeholder="e.g., Expected Close Date, Deal Stage, Probability"
                value={newField.name}
                onChange={(e) =>
      ...
      {/* Standalone Category Creator 👉 ADDED */}
      ...
      {/* Configured Fields grouped by Categories 👉 ADDED DND & Categorization Map */}
...
export default DealFieldSettings;
