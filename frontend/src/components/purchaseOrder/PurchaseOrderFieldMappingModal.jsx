...
// Field-mapping step for Purchase Order CSV import — same shape/behaviour as
// ItemFieldMappingModal.jsx (Products & Services), adapted to PO fields.
// Vendor Name + Item Name are the two fields the backend actually requires
// per row (see purchaseOrderController.bulkImportPurchaseOrders); rows that
// share the same PO Number + Vendor Name are grouped server-side into one
// multi-item PO.
const PurchaseOrderFieldMappingModal = ({
  isOpen: propIsOpen,
  onClose,
  csvHeaders,
  onImport,
  ...
  const handleFieldMappingChange = useCallback((csvHeader, poField) => {
  ...
  const handleImport = useCallback(() => {
    const validMapping = Object.entries(fieldMapping)
      .filter(([, poField]) => poField !== "")
      .reduce((acc, [csvHeader, poField]) => {
        acc[csvHeader] = poField;
        return acc;
      }, {});
                ...
                <div className="flex items-start">
                  ...
                  <div className="ml-3">
                    <p className="text-sm text-blue-800 font-inter">
                      ...
                      corresponding fields. Rows that share the same PO
...
export default PurchaseOrderFieldMappingModal;
