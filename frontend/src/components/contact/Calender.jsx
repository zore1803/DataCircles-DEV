...
const ContactCalendar = ({ contactId }) => {
  ...
  const fetchData = useCallback(async () => {
    try {
      const meetingsRes = await API.get("/meetings", { params: { contactId } });
      ...
      const tasksRes = await API.get(`/tasks/contact/${contactId}`);
      const tasksByDate = {};
  ...
  // Add this helper function at the top of ContactCalendar component
  ...
  const handleDayClick = (date) => {
    ...
    if (day >= today) {
      ...
      setQuickAddDate(dateString); // ✅ Now a string
      setQuickAddOpen(true);
    }
  };

  // handleQuickAddMeeting stays the same - it now receives a string
  const handleQuickAddMeeting = () => {
    setModalType("meeting");
    setModalMode("create");
    setCalendarDate(quickAddDate); // ✅ Now "2025-11-23"
    setSelectedMeeting(null);
    setModalOpen(true);
    setQuickAddOpen(false);
  };

  // handleQuickAddTask stays the same
  ...
  const handleSave = async (form, type) => {
    ...
    try {
      if (type === "meeting") {
        ...
        await API.post("/tasks", { ...form, contactId });
      }
