// One-off: creates an Organization + admin User + active Starter Subscription
// for local login testing. Idempotent: if the user already exists it just
// updates the password/subscription instead of erroring on the unique index.
//
// Usage: node scripts/createStarterLoginUser.js
...
async function main() {
  ...
  if (user) {
    ...
    org = await Organization.create({
      name: "Rohit Zore Starter Org",
      code: `RZ-${Date.now().toString(36).toUpperCase()}`,
    });
  ...
  // There's a hard unique index on `organization` in the subscriptions
  // collection (not just the partial one the schema comment describes), so
  // an org that already has a Subscription document — trial or otherwise —
  // must be updated in place rather than inserted fresh.
  const now = new Date();
  ...
  if (subscription) {
    ...
    console.log(`Updated existing subscription ${subscription._id} for organization ${org._id} to active Starter.`);
  } else {
    subscription = await Subscription.create({ organization: org._id, ...subFields });
    console.log(`Created active Starter subscription ${subscription._id} for organization ${org._id}.`);
  }
  console.log("\nLogin with:");
