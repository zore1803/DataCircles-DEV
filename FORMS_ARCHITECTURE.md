...
**No field-by-field merge exists in the v1 product surface.** An earlier pass of this decision treated "merge (field-by-field reconciliation)" as a third manual outcome and built `duplicateResolutionService.mergeIntoExisting()` for it. That function is **not removed** from the codebase (it's a legitimate future capability, and the schema's `DuplicateReview.decision` enum still has room for `"merged"`), but it is **not wired into any v1 flow** and the Review UI does not expose it — per this clarification, the only two choices a user is ever given are Keep Both / Keep Existing.
